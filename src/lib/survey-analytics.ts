...
export function parseSurveyRows(rows: RawSurveyRow[]): ParsedSurveyRow[] {
  return rows.map((r) => {
    let answersObj: Record<string, string | string[] | number> = {};
    try {
      answersObj = typeof r.answers === "string" ? JSON.parse(r.answers) : (r.answers as any) || {};
    } catch {
      answersObj = {};
    }
    return {
...
export function computeSurveyStats(parsed: ParsedSurveyRow[], total: number) {
...
// ---------------------------------------------------------------------------
// Helpers internos de conteo
// ---------------------------------------------------------------------------

function countValues(
  parsed: ParsedSurveyRow[],
  questionId: string
...
function avgScale(parsed: ParsedSurveyRow[], questionId: string): number {
  let sum = 0;
  let count = 0;
  for (const p of parsed) {
    const v = p.answers[questionId];
